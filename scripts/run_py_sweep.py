...
from conop_py.cost import (
    build_section_observations, ordinal_misfit,
...
def run_sweep(mode: str, use_anchors: bool = True) -> list[dict]:
    ...
    # 预计算支持度权重（weighted 模式）
    ps = build_pairwise_support(obs) if use_weighted else None
    ...
    for tag, ratio, startemp, steps in PARAM_GROUPS:
        for run_idx, seed in enumerate(SEEDS, 1):
            ...
            # 统一用 Ordinal 打分，方便与原版对比
            ord_score = ordinal_misfit(res.best_sequence, section_obs)
            wt_score = (
                weighted_ordinal_misfit(res.best_sequence, section_obs, ps)
                if ps else None
            )

            print(f"best_fit(reported)={res.best_fit:.2f}  "
                  f"ordinal={ord_score:.2f}"
                  + (f"  weighted={wt_score:.4f}" if wt_score is not None else ""))
...
def main() -> None:
    ...
    ap.add_argument("--mode", choices=["ordinal", "weighted", "both"], default="ordinal")
    ...
    if args.mode in ("weighted", "both"):
        ...
        save_csv(results_wt, out_dir / "sweep_weighted.csv")
        if orig_rows:
