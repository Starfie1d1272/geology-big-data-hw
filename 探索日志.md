...
| 项 | 文件 | 内容 |
|----|------|------|
| **B6** 增量 ordinal | `cost.py` `FastOrdinalState.trial_move` | 差分公式 O(n_s) 算 Δordinal，跳过 sort+merge；只重算 ev 所在的 1-4 个 section 而不是全部 12 个 |
| **B7** NumPy pos 数组 | 同上 | `_pos_arr: np.ndarray[int32]` 全局位置数组，配合 numba 用；同步维护 `_pos_dict` 兼容旧代码 |
| **B8** 早停 | `anneal.py` `_anneal_fast_ordinal` | `cfg.early_stop_patience` 控制——连续 N 步 best 无改善退出，典型省 15-30% 时间 |
| **B9** numba JIT | `cost.py` `_count_inversions` | `@numba.njit(cache=True)` 装饰内层逆序计数，纯 Python fallback 自动生效 |
| **B10** 多进程多重启 | `scripts/run_multistart.py` | `multiprocessing.Pool` 并行 N 个独立 SA，每个不同 seed，输出 bestsoln + summary.csv + manifest |
...
| +B6+B7 差分 | 89,824 | ~2 s | ~25 s |
| +B9 numba | 170,764 | **~1 s** | **10.3 s** |
...
```
tests/test_regression.py           # 7 个回归测试，固定 ordinal=367 / level=340 / eventual=491
                                   # + 300 次随机扰动 + revert 一致性检验
scripts/benchmark_sa.py            # 单次 SA 计时
scripts/profile_sa.py              # cProfile 热路径分析
scripts/run_multistart.py          # 多进程多重启入口
conop_py/cost.py FastOrdinalState  # 增量 ordinal 状态机（用法见 docstring）
```

### 50 次重启实测（baseline 参数，2026-05-27）

`results_py/multistart/2026-05-26_185637Z_b-region-test/` 已存档：
