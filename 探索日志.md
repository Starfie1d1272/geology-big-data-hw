...
| 类别 | 事件 | rank 中位数 | 95% CI 宽度 | 含义 |
|------|------|:----------:|:----------:|------|
| 高置信 | `Pachydiscus_FAD` | 100 | 4 | 🟢 地层标志可靠 |
| 高置信 | `Hoploscaphites_LAD` | 119 | 4 | 🟢 |
| 高置信 | `KT_EVT` | 119 | 5 | 🟢 K-Pg 界线稳定 |
| 矛盾 | `Anapachydiscus_LAD` | 54 | **47** | 🔴 横跨半个序列 |
| 矛盾 | `Baculites_LAD` | 56 | **46** | 🔴 数据打架 |
| 矛盾 | `Phylloceras_(H.)_FAD` | 48 | **41** | 🔴 |
| 矛盾 | `Hypophylloceras_FAD` | 74 | 34 | 🟡 |
| 矛盾 | `Maorites_FAD` | 45 | 31 | 🟡 |

**地质意义**：LAD 类事件普遍置信度高（窄区间），FAD 类事件矛盾大。说明**灭绝层面比出现面在数据中更一致**——这与菊石作为远洋漂浮生物、全球同时灭绝的模式一致。三个矛盾最严重的事件可作为"建议野外重新审查层位"的候选。
...
**关键结果**：50 个解在 2D 平面上聚成一个紧密的簇（距离范围 [0, 0.138]），**没有出现分离的解簇**。颜色（best_fit）从绿到红渐变，但没有结构性分离。

**地质意义**：虽然 best_fit 有 238-258 的波动（约 8% 的差异），但这些解在结构上高度相似——Spearman ρ 最低也是 0.862。说明**不存在多个质量相当的竞争性解**，解空间呈单峰分布。ratio_095 的 265/220/220 尽管 best_fit 差异大，但它们之间的解结构差异可能不如预期的大。
...
**关键结果**：
- TEMP=100：初始接受率仅 ~40%，过快冻结 → 最终 best_fit=242（最差）
- TEMP=250：初始接受率 ~55%，平滑下降 → 最终 best_fit=170（最佳）
- TEMP=500：初始接受率 ~70%，前 200 步几乎完全随机行走 → 最终 best_fit=164（略好于 250）

**地质意义**：接受率在 ln(T) 的 50% 交叉点几乎一致（T≈80），说明**不同初始温度的核心搜索阶段长度相同**。TEMP=500 多出的前期高温扰动对最终质量帮助有限（164 vs 170，量级差异远小于 seed 间的波动）。
